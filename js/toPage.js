function page_ctrl(data_obj) {
  ...
  function page_even() {
    ...
    function change_content() {
      ...
      var page_content='<ul style="width: 300px;margin: 10px auto;" class="live">';//当前页内容

      for(var i=0;i<per_num;i++){
      	if(title[i]==null){title[i]="暂无";}
      	if(time[i]==null){time[i]="暂无";}
      	if(info[i]==null){info[i]="暂无";}
      	if(picture[i]==null){picture[i]="暂无";}
      	if(title[i]!="暂无"){
        page_content+='<li class="artive-list">'
        +'<div class="title">'+title[i]+'</div>'
        +'<div class="time">'+time[i]+'</div>'
        +'<div class="info">'+info[i]+'</div>'
        +'<div class="tag" onClick="go('+i+')">详情</div>'
        +'</li>';
        }else{
        page_content+='<li class="artive-list">'
        +'<div class="title">'+title[i]+'</div>'
        +'<div class="time">'+time[i]+'</div>'
        +'<div class="info">'+info[i]+'</div>'
        +'<div class="tag" onClick="go('+i+')">暂无</div>'
        +'</li>';	
        }
      }
    ...
    append_html+='<button class="next_page">下一页</button><span class="page_total">共 '+total_page+' 页, 到第</span><input class="input_page_num" type="text" value="'+(inp_val-1)+'"><span class="page_text">页</span><button class="to_page_num">确定</button>';
